...
const TodoItem = ({ theme, todo, dispatch }: TodoItemProps) => {
  ...
  return (
    <motion.div
      className={`flex justify-between items-start cursor-default bg-white rounded-lg shadow-lg px-5 py-4 mb-4 ${
        todo.completed ? "" : "hover:shadow-xl"
      } transition-shadow duration-200 dark:bg-gray-900`}
    ...
    >
      <div className={`flex flex-col gap-2 flex-wrap w-9/12`}>
        <h2
        ...
        <div className="flex justify-start items-center gap-3 mt-3">
          ...
          >
            <FaPen />
            <span className="absolute hidden sm:block bottom-full mb-2 px-4 py-2 text-sm text-white bg-gray-800 dark:bg-gray-700 rounded scale-0 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition duration-200 pointer-events-none whitespace-nowrap z-50">
              Update task
            </span>
          </button>
          <button
